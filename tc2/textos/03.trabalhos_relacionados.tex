...
O método de reconhecimento de voz utilizado é independente do texto da amostra, são extraídas as características do sinal através da técnica de \textit{cepstral melodic analysis}. Após as frequências são convertidas para escala de mel que é mais apropriada para voz então é calculado um \textit{delta mel frequency cepstral coefficients} (DDMFCC) para cada amostra, que então podem ser utilizados como entrada para um algoritmo de classificação supervisionada como: mínima distância média ou k-NN.

Para o reconhecimento facial é utilizado a técnica \textit{Scale Invariant Feature Transform} (SIFT), publicada por David Lowe em 1999. Esta técnica faz a extração de pontos chave de uma imagem a fim de produzir uma descrição. Estas características não são afetadas pelo tamanho da imagem, sua orientação ou mudanças de iluminação, o que a torna robusta para descrição de faces. A técnica de SIFT resulta em um vetor de características, onde para que duas faces sejam consideradas da mesma pessoa ambos vetores precisam estar em uma distância próxima um do outro. Para a tarefa de reconhecimento das características faciais extraídas também são utilizadas as técnicas de mínima distância média ou \textit{K-Nearest-Neighbour} (K-NN).

E finalmente para o reconhecimento de íris, é proposta uma técnica que explora a sua distribuição de cores. Uma imagem da íris é utilizada para o reconhecimento, e são consideradas somente a parte colorida da imagem, descartando a pupila no centro da imagem. Esta imagem é então dividida em quatro setores e para cada um é calculado seu histograma, e então o resultado deste processamento é obtido concatenando os quatro histogramas em um vetor de cores. Para a identificação da íris a classificação de mínima distância sobre o vetor de cores pode ser usada.

Com todas as três características biométricas processadas o sistema realiza o processo de combinação. Este processo é realizado pode obter melhores resultados com as entradas das características originais de biometria, porém a combinação neste nível é desafiante pois os dados são incompatíveis em sua primeira forma, por isto no sistema proposto foi considerado a combinação no módulo de decisão. Foram explorados diferentes estratégias de combinação que são: \textit{majority voting}, \textit{behavior knowledge space method}, \textit{AND/OR rules and weighted based on Dempster-Shafter theory of evidence}. O escolhido para o sistema foi o de \textit{majority voting} aplicado sobre as três características.

Foram realizados testes utilizando o sistema desenvolvido, e cada um dos três métodos de reconhecimento obteve resultados equivalentes ao estado da arte, e o sistema desenvolvido obteve uma taxa de reconhecimento de aproximadamente 90\%. O sistema desenvolvido se demonstrou eficiente para a autenticação de usuários, e o sistema pode ser melhorado em trabalhos futuros integrando outro reconhecimento biométrico como o de digitais.

\section{A secured mobile-based password manager}
O trabalho apresentado por \cite{Agholor:2016}, tem como objetivo principal o desenvolvimento de um sistema de gerenciamento de senhas seguro para dispositivos móveis. O sistema proposto é dividido em quatro módulos, sendo eles: modulo de credenciais, que é responsável para a criação de uma senha mestre, uma dica de recuperação, perguntas e respostas de verificação, além de informações do usuário.

O módulo de gerenciamento é responsável pelo processo de geração de senhas, além dos processos de \textit{backup}, restauração e atualizações. Já o módulo de segurança é responsável pela cifragem e decifragem das credenciais fornecidas pelo módulo de gerenciamento, para isto é aplicado o esquema de criptografia \textit{Advanced Encryption Standard} (AES).

Por fim o módulo que faz o armazenamento das credenciais no formato de arquivo descentralizado pelo trabalho. A segurança do sistema foi avaliada utilizando dispositivos móveis com senhas armazenadas de forma criptografada em cada um deles, então foram feitos tentativas de ataques contra estas senhas cifradas salvas em formato de arquivo. O resultado obtido foi que em 100 credenciais armazenadas em 10 diferentes dispositivos, apenas um deles foi capaz de descobrir a senha correta e invadir a conta do usuário. O trabalho proposto resultou em um sistema capaz de aumentar a segurança em sistema de armazenamento de senhas em dispositivos móveis.


\section{Cloud password manager using privacy-preserved biometrics}
O trabalho apresentado por \cite{yang:2014}, tem como objetivo principal o desenvolvimento de um sistema de gerenciamento de senhas em nuvem utilizando reconhecimento biométrico. O sistema proposto utiliza uma forma de autenticação em duas etapas, onde é utilizado uma senha mestre juntamente com biometria, este processo de autenticação está integrado, e não retorna informações para um usuário mal-intencionado sobre cada processo separadamente.

O sistema faz a proteção das biometrias coletadas, onde elas são processadas utilizando uma técnica de transformação única, dificultando que elas sejam recuperadas pois não existe o processo inverso.
As leituras biométricas e a senha mestre do usuário estão protegidas, e o sistema funciona de uma maneira em que caso a biometria ou a senha mestre sejam descobertas não seja possível a descoberta do outro. O sistema utiliza a nuvem para poder realizar a sincronização das senhas entre diversos clientes que estejam conectados ao servidor, e o tipo de biometria utilizado no sistema é o reconhecimento de digitais.

Para se autenticar no sistema o usuário precisa primeiro fornecer a sua senha e a leitura biométrica, que passa pelo processo de transformação e combinação, e após são comparados com as credenciais do usuário no servidor. Após a autenticação o usuário pode utilizar o sistema de gerenciamento para criar, editar e remover as senhas cadastradas. Cada senha cadastrada no sistema é combinada com a chave mestre e biometria do usuário e cifradas antes de serem armazenadas. O sistema proposto apresenta uma solução segura para o armazenamento seguro de senhas, onde é feito o uso de autenticação de duas etapas com biometria e chave mestre para sincronizar senhas de forma segura entre servidores e clientes.



\section{Discussão}
Os trabalhos relacionados serviram de base para o desenvolvimento deste trabalho de conclusão de curso. Buscou-se analisar artigos que tivessem o objetivo similar, porém que utilizaram diferentes biometrias, técnicas de reconhecimento e técnicas de geração e armazenamento de senhas, a fim de definir as melhores combinações para o desenvolvimento do sistema proposto.

Entre os oito trabalhos apresentados o que mais se aproxima do objetivo deste trabalho foi o de \cite{aliasgari:2015}, que tem como objetivo principal desenvolver um sistema de gerenciamento de senhas seguro para dispositivos móveis, utilizando reconhecimento de voz e de fala para realizar a autenticação e criptografia para armazenar de forma segura as senhas. A única diferença é que o processamento das biometrias é feito pelos servidores e não diretamente no dispositivo móvel.

A Tabela \ref{comparativo} mostra um comparativo entre os trabalhos relacionados ao uso de reconhecimento biométrico analisados, levando em consideração as características biométricas utilizadas, as técnicas de reconhecimento biométrico.

\begin{table}[H]
\centering
\caption{Comparativo entre os trabalhos de reconhecimento biométrico.}
\label{comparativo}
...
%\hline \cite{yang:2014}     & Digitais  & Desconhecido \\ 
\hline
\end{tabular}
\fonte{(AUTORES, 2018)}
\end{table}

Na Tabela \ref{comparativo}, a coluna "Biometria" refere-se as características biométricas utilizadas no trabalho. Todos os trabalhos realizaram a tarefa de reconhecimento de uma ou mais biometria, e os trabalhos \cite{gofman:2016}, \cite{Lathika:2014} e \cite{barbu:2015} aplicaram este reconhecimento em um sistema de autenticação, enquanto os outros tem como objetivo apenas o reconhecimento. Também é possível perceber que os trabalhos possuem características biométricas em comum que são: faces e voz.
...
A Tabela \ref{comparativo2} mostra um comparativo entre os trabalhos relacionados ao gerenciamento de senhas de forma segura, levando em consideração as formas de autenticação, plataforma, algoritmos de criptografia e se possui gerador de senhas.

\begin{table}[H]
\centering
\caption{Comparativo entre os trabalhos de gerenciamento de senhas.}
\label{comparativo2}
\begin{tabular}{|p{4cm}|p{3cm}|p{2cm}|p{3cm}|p{2cm}|p{2cm}|}
\hline Trabalho Relacionado & Autenticação & Plataforma & Gerador senhas & Criptografia  \\ 
\hline \cite{aliasgari:2015}& Senha, Biometria      & Mobile     & Não            & AES-256       \\
\hline \cite{Agholor:2016}  & Senha, Biometria      & Mobile     & Sim            & AES           \\
\hline \cite{yang:2014}     & Senha, Biometria      & Web        & Não            & AES-256       \\
\hline
\end{tabular}
\fonte{(AUTORES, 2018)}
\end{table}
...
Na Tabela \ref{comparativo2}, a coluna "Autenticação" refere-se as características biométricas utilizadas no sistema. Todos os trabalhos realizaram a autenticação através de reconhecimento de reconhecimento biométrico e senhas. A coluna "Plataforma" refere-se a plataforma para qual o sistema foi desenvolvido. Também foram analisados os trabalhos a fim de verificar o uso de geração de senhas complexas, e quais algoritmos de criptografia são empregados.